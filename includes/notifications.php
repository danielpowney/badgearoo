...
function ub_assignment_moderation_notification( $assignment_id, $condition_id, $user_id, $type, $value, $created_dt ) {
	...
	if ( ! $general_settings['ub_assignment_auto_approve'] && $general_settings['ub_assignment_notify_moderators'] ) {
			
		$from_email = $general_settings['ub_assignment_moderation_notification_email'];
		$from_name = $general_settings['ub_assignment_moderation_notification_from'];
		$subject = $general_settings['ub_assignment_moderation_notification_subject'];
		$heading = $general_settings['ub_assignment_moderation_notification_heading'];
		$message = $general_settings['ub_assignment_moderation_notification_template'];
		...
		$assignment_moderation_link = admin_url() . 'edit.php?post_type=badge&page=ub_assignments&status=pending';
		
		$assignment_details = '';
		if ( $type == 'badge' ) {
			$badge = User_Badges::instance()->api->get_badge( $value );
			$assignment_details = sprintf( __( 'Badge: %s', 'user-badges' ), $badge->title ); 
		} else {
			$assignment_details = sprintf( __( 'Points: %d', 'user-badges' ), $value );
		}
		...
		$emails = array();
		$moderators = preg_split( '/[\r\n,]+/', $general_settings['ub_assignment_moderators'], -1, PREG_SPLIT_NO_EMPTY );
		...
		add_filter( 'wp_mail_content_type', 'ub_set_html_content_type' );
		foreach ( $emails as $moderator_email ) {
			@wp_mail( $moderator_email, wp_specialchars_decode( $subject ), $message, $message_headers );
		}
		remove_filter( 'wp_mail_content_type', 'ub_set_html_content_type' );
 ...
 * @return string
 */
function ub_set_html_content_type( $content_type ) {
	return 'text/html';
}